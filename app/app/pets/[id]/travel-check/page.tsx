import {notFound,redirect} from 'next/navigation';import {sessionProfile} from '@/lib/access';import {TravelCheck} from '@/components/travel-check';import {PremiumGate} from '@/components/premium-gate';
...
if(!pet)notFound();
return <main className="min-h-screen bg-paper px-5 py-10 text-[#272621] [color-scheme:light]"><div className="mx-auto max-w-[480px]"><a className="text-clay" href="/app">← Dashboard</a><h1 className="mt-8 text-3xl">EU travel check</h1><p className="mt-2 text-sm text-black/60">Check {pet.name}&apos;s record against the EU Pet Travel Scheme basics before you book.</p>{session.premium?<TravelCheck petName={pet.name} rabiesVaccinatedAt={pet.rabies_vaccinated_at} microchipNumber={pet.microchip_number}/>:<PremiumGate trigger="eu_travel" petName={pet.name} description={`Check ${pet.name}'s microchip and rabies timing against the EU Pet Travel Scheme before you book.`}/>}</div></main>}
