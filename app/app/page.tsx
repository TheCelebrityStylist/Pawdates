...
s.from('treatment_log').select('done_at,was_overdue,treatment:treatments(name,pet_id)').order('done_at',{ascending:false}).limit(240),
s.from('routine_checks').select('checked_at,checked_by,pet_id,routine_item:routine_items(label)').order('checked_at',{ascending:false}).limit(160),
...
// Puppy-stage status per pet (migration 0020 may be unapplied → fall back).
const [pmR,priR,prlR]=petIds.length?await Promise.all([
s.from('puppy_mode').select('pet_id,enabled,band,new_to_household').in('pet_id',petIds),
s.from('puppy_routine_items').select('pet_id').eq('active',true).in('pet_id',petIds),
s.from('puppy_routine_log').select('pet_id,item_id').in('pet_id',petIds).eq('done_for_date',today)
]):[{data:[],error:null},{data:[],error:null},{data:[],error:null}];
const pmRows=(pmR.error?[]:pmR.data||[]) as {pet_id:string;enabled:boolean;band:string;new_to_household:boolean}[];
...
for(const w of (weightLogs||[]).filter(w=>w.pet_id===pet.id).slice(0,40))events.push({id:`w-${pet.id}-${w.recorded_at}`,date:w.recorded_at,kind:'weight',label:'Weight logged',detail:`${w.weight_kg} kg`,wasOverdue:null});
for(const v of (vetVisits||[]).filter(v=>v.pet_id===pet.id).slice(0,40))events.push({id:`v-${pet.id}-${v.date}`,date:v.date,kind:'visit',label:v.reason,detail:v.cost_cents?`€${(v.cost_cents/100).toFixed(2)}`:'Vet visit',wasOverdue:null});
for(const t of (treatmentLogs||[]).filter(t=>{const tr=Array.isArray(t.treatment)?t.treatment[0]:t.treatment;return tr?.pet_id===pet.id}).slice(0,40)){const tr=Array.isArray(t.treatment)?t.treatment[0]:t.treatment;events.push({id:`t-${pet.id}-${t.done_at}`,date:t.done_at,kind:'treatment',label:tr?.name||'Treatment',detail:'Marked done',wasOverdue:t.was_overdue})}
for(const c of (routineChecks||[]).filter(c=>c.pet_id===pet.id).slice(0,40)){const item=Array.isArray(c.routine_item)?c.routine_item[0]:c.routine_item;events.push({id:`c-${pet.id}-${c.checked_at}`,date:c.checked_at,kind:'checkoff',label:item?.label||'Checked off',detail:`By ${c.checked_by}`,wasOverdue:null})}
...
lifeEventsByPet[pet.id]=events.sort((a,b)=>new Date(b.date).getTime()-new Date(a.date).getTime()).slice(0,60);
}
