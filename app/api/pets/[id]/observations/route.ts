import {z} from 'zod';import {sessionProfile} from '@/lib/access';import {fail,ok} from '@/lib/http';import {observationSchema} from '@/lib/care-profile';import {ownerOfPet} from '@/lib/household';
...
const {data,error}=await session.client.from('observation_log').insert({pet_id:id,user_id:ownerId,actor_user_id:session.user.id,tag:body.tag,note:body.note||null}).select('id,tag,note,created_at').single();
