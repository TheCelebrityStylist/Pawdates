...
export async function GET(_req: Request, {params}: {params: Promise<{id: string}>}) {
  try {
    ...
    const {data, error} = await session.client.from('providers').select('*').eq('pet_id', id).order('type').order('name');
    if (error) return fail('query_failed', error.message, 500);
    return ok({providers: (data || []) as Provider[]});
  } catch (e) {
...
export async function POST(req: Request, {params}: {params: Promise<{id: string}>}) {
  try {
    ...
    const {data, error} = await session.client
      ...
      .select('*')
      .single();
    if (error || !data) return fail('save_failed', error?.message || 'Could not save provider', 500);
    return ok({provider: data as Provider}, 201);
  } catch (e) {
    return fail('invalid_request', e instanceof Error ? e.message : 'Invalid request', 400);
  }
}
